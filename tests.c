...
void test_source_init_free() {
  ...
  assert_okb(src1.buf_data != NULL &&
             strncmp("avocado", (char *)src1.buf_data, src1.buf_len) == 0);
...
void test_token_init() {
  ...
  assert_eql(token.offset, 0L);
  assert_eql(token.length, 0L);
  assert_eql(token.auxlen, 0L);
}
...
void test_token_next_singlechar() {
  ...
  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_CALL_S);
  assert_eql(token.offset, 0L);
  assert_eql(token.length, 1L);

  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_LIST_S);
  assert_eql(token.offset, 1L);
  assert_eql(token.length, 1L);

  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_CALL_S);
  assert_eql(token.offset, 2L);
  assert_eql(token.length, 1L);
  avoc_source_free(&src);
  ...
  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_CALL_E);
  assert_eql(token.offset, 0L);
  assert_eql(token.length, 1L);

  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_LIST_E);
  assert_eql(token.offset, 1L);
  assert_eql(token.length, 1L);
...
void test_token_next_comments() {
  ...
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_COMMENT);
  assert_eql(token.offset, 8L);
  assert_eql(token.length, 7L);

  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_COMMENT);
  assert_eql(token.offset, 16L);
...
void test_token_next_str_lit() {
  ...
  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_LIT_STR);
  assert_eql(token.offset, 0L);
  assert_eql(token.length, 8L);
  assert_eql(token.auxlen, 6L);
  avoc_source_free(&src);
  ...
  assert_eql(token.offset, 0L);
  assert_eql(token.length, 3L);
  assert_eql(token.auxlen, 1L);

  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_LIT_STR);
  assert_eql(token.offset, 4L);
  assert_eql(token.length, 3L);
  assert_eql(token.auxlen, 1L);
  avoc_source_free(&src);
...
void test_token_next_num_lit() {
  ...
  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_LIT_NUM);
  assert_eql(token.offset, 0L);
  assert_eql(token.length, 4L);
  avoc_source_free(&src);
...
void test_token_next_nilbol_lit() {
  ...
  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_LIT_BOL);
  assert_eql(token.offset, 0L);
...
void test_token_edge_cases() {
  ...
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_LIT_NUM);
  assert_eql(token.offset, 11L);
  assert_eql(token.length, 1L);

  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_COMMENT);
  assert_eql(token.offset, 13L);
  assert_eql(token.length, 13L);

  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  assert_eq(token.type, TOKEN_LIT_NUM);
  assert_eql(token.offset, 27L);
  assert_eql(token.length, 1L);
  avoc_source_free(&src);
...
void test_parse_lst_lit() {
  ...
  load_string(&src, "[first second 3]");
  avoc_item_init(&item);
  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  status = avoc_parse_lit(&src, &token, &item);
  assert_okb(status == OK);
  assert_eq(item.type, ITEM_LIT_LST);
  assert_okb(item.as_list != NULL);
  list = item.as_list;

  assert_eq(list->head->type, ITEM_SYM);
  assert_eqs(list->head->as_str, "first");

  assert_okb(list->head->next_sibling != NULL);
  assert_eq(list->head->next_sibling->type, ITEM_SYM);
  assert_eqs(list->head->next_sibling->as_str, "second");

  assert_eq(list->tail->type, ITEM_LIT_I32);
  assert_eq(list->tail->as_i32, 3);
  ...
  assert_okb(list->tail->type == ITEM_LIT_LST);
  avoc_item *nested_head = list->tail->as_list->head;
  assert_eq(nested_head->type, ITEM_SYM);
  assert_eqs(nested_head->as_str, "second");
...
void test_parse_sym_with_ord_type() {
  ...
  load_string(
      &src,
      "sym1:type1 SYM2:TYPE2 $sym3:$type3 \xF0\x9F\xA5\x91:\xF0\x9F\xA5\x91");
  avoc_item_init(&item);
  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  status = avoc_parse_sym(&src, &token, &item);
...
void test_parse_calls() {
  ...
  status = avoc_next_token(&src, &token);
  assert_okb(status == OK);
  status = avoc_parse_list(&src, &token, &list, TOKEN_CALL_E);
  assert_okb(status == OK);
  ...
  assert_okb(list.tail != NULL);
  assert_okb(list.tail->type == ITEM_CALL);
...
void test_parse_sym_with_composed_type() {
  ...
  assert_okb(type->tail != NULL);
  assert_eq(type->tail->type, ITEM_CALL);
...
void test_parse_source() {
  ...
  status = avoc_parse_source(&src, &list);
  assert_okb(status == OK);
  assert_okb(list.head->type == ITEM_CALL);
...
int main() {
  ...
  trun("test_parse_lst_lit", test_parse_lst_lit);
  ...
  trun("test_parse_lists", test_parse_calls);
